...
void Open() {
  ...
  if (!opening) {
    ...
    if (!speedOverride) {
      term.println("OPEN USLOW + PUSH (" + String(ULTRASLOW + PUSH) + ")");
      analogWrite(pwm, ULTRASLOW + PUSH);  // push is 0 at first and will be adjusted if needed (in the while loop just below)
    } else {
      term.println("OPEN OVERRIDE SPEED VAL:" + String(speedVal));
    ...
    {
      if (millis() - Start > 20000 && contactClosed && millis() - lastPushIncrease > 20000) {
        int prev = PUSH;
        PUSH += 10;
        if (!speedOverride) {
  ...
  opening = false;  // has to be here despite pb
  speedOverride = false;
...
void Close() {
  ...
  if (contactClosed) {
    ...
    simpleClose();  // give extra push in case this is the intended request.
    if (!speedOverride) {
  ...
  if (!closing) {
    ...
    simpleClose();
    term.println("speedOverride ? " + String(speedOverride ? "YES" : "NO"));
    if (!speedOverride) {
    ...
    while (!contactClosed && !stopped && !EMstop && millis() - Start < 1200000 * 2)  // 1,200,000 millis = 20 min * 2 = 40
    {
      if (millis() - relativeStart > operationTime)  // 600,000 = 10 min
      {
        relativeStart = millis();
        ULTRASLOW = ULTRASLOW < LEARNING_SPEED_LIMIT ? ULTRASLOW + 50 : ULTRASLOW;  // seems it's not moving, so go a bit faster
