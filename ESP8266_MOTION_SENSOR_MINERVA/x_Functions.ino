...
{
  Serial.println("sending " + var + " to the Hubs");
  ...
  else if (debugData.length() == 0) // add last boot time info to the main string either after reboot or after string data reset
  {
    debugData.concat(preserved + "\n");
  }
  else
...
void Reset() {
  ESP.reset(); // not using ESP.restart() for it makes the chip hang in flash mode on ESP8266
}
