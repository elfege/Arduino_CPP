void setup()
{
  ...
  // create OTA name
  const char *NameOTA = "AC_REMOTE_ESP8266";
...
//  ESP.wdtDisable(); // disable the software watchdog, which triggers the hardware watchdog, which will trigger if software is not fed (ESP.wdtFeed()) before 6 seconds
  ...
  acOff(); // must be turned off to reset turboIsOn to proper value regarding current state since we don't have operation feedback. 
}
