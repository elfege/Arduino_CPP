...
void Open() {
  ...
  if (!opening)
  {
    ...
    if (ran == true && !isOpen)
    {
      while (!isOpen() || millis() - Start < 2000) // give another2 seconds unconditional push if it's still returning not open
      {
        Move(dir);
      }
    }
...
void Close()
{
  ...
  if (!closing)
  {
    ...
    bool ran = false;
    unsigned long Start = millis();
    while (isFullyOpen() || millis() - Start < 2000) // give a minimum push of 2 seconds
    {
      ran = true;
      Move(dir);
      // no call for master() here
...
void forceOpen(int dur)
{
  ...
  stopped = false;
  int i = 0;
  dir = "A"; //Rotate stepper motor in clock wise direction
  unsigned long Start = millis();
  while (millis() - Start < dur && !stopped)
  {
    Move(dir);
