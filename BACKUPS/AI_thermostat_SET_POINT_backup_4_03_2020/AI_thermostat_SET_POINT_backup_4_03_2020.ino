    ...
    for(ms != 0; m < ms && !learning; m++) // location mode loop
    {
...

        {
            ...
            for(s != 0; i < s; i++) {
                ...
                {
                    log.warn "value $ref not replaced in ${dbX[a]} at user's request"
                    //return
                }
                else // if this is not to be saved, then just replace the value
                {
                    ...
                    {
                        if(now() - startDebug > delay) {log.info "${dbX[a]} becomes $insert"}
                        dbX[i] = insert
                    }
                    if(now() - start > timer)
                    {
        ...
        else   // depending on whether we want to save old data, or not, we apply a different writing method: replacement or increment         
        {
            ...
            for(s!=0;i<s;i++)
            {
                int a = 0
                h = 10
                for(s!=0;i <= maxTemp;a++) // for each i (outside temp max = 120) we create all variations of humidity (max = 100)
        ...
        state.db << newIt
        state.backupRepair = state.db // backup the db for upcoming comparisons 
