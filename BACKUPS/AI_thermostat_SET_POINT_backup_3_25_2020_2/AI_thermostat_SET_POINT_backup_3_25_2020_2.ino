...
def pageConfig() {
    ...
    dynamicPage(pageProperties) {
        ...
        section("Home Location Modes Management")
        {
            ...
            {
                input "checkConsistency", "button", title:"Update New Modes"
            }
        }
...
"""
    ...
    else if(state.db.size() > baseMode.size())
    {
        ...
        for(s!=0;i<s;i++)
        {
            ...
            inDb = state.db.find{it.key == "${location.modes[i]}"}?.key
            //log.debug "inDb = ${inDb}"
            boolean inBM = baseMode.find{it == "${location.modes[i]}"}
            boolean noLongerInBaseMode = !inBM && inDb
            log.warn "noLongerInBaseMode = $noLongerInBaseMode or not applicable (${location.modes[i]})"
           
            
            if(noLongerInBaseMode)
            {
                log.info "deleteding ${location.modes[i]} from database"
                state.db.remove("${location.modes[i]}") 
            } 
        }
