    ...
    dynamicPage(pageProperties) {
        ...
        section()
        {
            ...
            {   
                ...
                descriptiontext "logging enabled"
            }
            else 
            {
...
def initialize(){
    ...
    log.into "initializing ${app.label}..."
    state.EnableDebugTime = state.EnableDebugTime ? state.EnableDebugTime = now() : state.EnableDebugTime
...
This event was triggered by this app"""
    ...
    ignoreMessage = ignoreMessage != "null" ? descriptiontext(ignoreMessage) : ""
    ...
    if(!restrictedMode)
    {
        if(!state.paused )
        {
            ...
            {
                descriptiontext "Not learning, override due to 'auto' mode"
            }
        }
        else 
        {
            descriptiontext "app paused"
        }
    }
    else 
    {
        descriptiontext "restricted mode"
    }
    ...
    {      
        ...
        if(baseMode[a] == location.mode)
        {
            ...
            {    
                ...
                {
                    log.warn """setPointHandler() took more than $safetyDelay seconds to execute.
Stopping operation and restoring backup"""
                    state.db = backup
                    return
                }
...
def descriptiontext(message){
    //state.enablelogging = true
    if(enabletext) descriptiontext message
