...
void Refresh() {
  stateSwitch = CurtainsAreOpen ? "switch on" : "switch off";
  buildDebug(stateSwitch);
  delay(500);
  stateCurtain = CurtainsAreOpen ? "curtains open" : "curtains closed";
  buildDebug(stateCurtain);

  buildDebug("CurtainsAreOpen = " + String(CurtainsAreOpen ? "true" : "false"));
}
...
void Stop() {
  ...
  buildDebug(var);
  delay(500); // leave time to the stopped state to appear (before it updates with on/off open/closed if not stopped by user intervention)
}
