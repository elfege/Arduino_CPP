...
void setup() {
  ...
  Clamp.write(ClampClosed); // keep it closed to avoid throwing a possible can away during tests operations
  ...
  TryAgain = false; // only place to reset this value after completion
  //of the whole process the setup() function is called
...
void loop() {
  ...
  if (time >= TimeB4Tests) {
    ...
    /// KEEP ALIVE
    // if power is off test connection
    if (!powerIsON()) {
      Serial.println("KEEP ALIVE --- TESTING CONNECTION");
      PingTest();
      delay(10);
      blinks = 10;
      lapse = 100;
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {
          ...
          client.print(F(
                         ...
                         "<input type=submit value='FULL TEST' style=width:120px;height:30px;color:blue onClick=location.href='/AllTest'>"
                         ...
                         "<br/>"
                         "<input type=submit value='CLOSE PLATE GRABBER' style=width:250; height:30px onClick=location.href='/grabberclose'>"
                         "<input type=submit value='OPEN PLATE GRABBER' style=width:250; height:30px onClick=location.href='/brabberopen'>"
                         "<br/>"
          ...
          else if (currentLine.startsWith("GET /grabberopen")) {
            Grabber.write(GrabberOpen);
          }
          else if (currentLine.startsWith("GET /closeplatebarrier")) {
...
void LocatePlate() {
  toomanygrabbed = false; // reset of this value
  ...
  if (!IsLastPlate() && locatetries < 10) {
    ...
    while (1) {
      ...
      if (TryAgain) {
        ...
        if (pushtries <= Max && !toomanygrabbed) {
        ...
        else if (toomanygrabbed && pushtries <= Max) {
          toomanygrabbed = false; // allow next cycle to treat a different type of prob.
          pos += pushtries; /// if here then push is what failed
          Serial.println("");
  ...
  Serial.println("");
  if (!platehasbeenpushed) {
    Serial.println("WAITING FOR PLATE TO BE PUSHED AWAY");
  }
  else {
    Serial.println("platehasbeenpushed returns TRUE -- ERROR --- ");
  }
...
void PushPlateOut() {
  ...
  int ArmLiftCurr = ArmLift.read(); // record current armlift position

  //delay(8000);// give it a secure 8 seconds of push
  // move forward until good grasp is reached
  while (elapsed < TimeLimit && !plategrab()) {
    ...
    // continue adjusting arm's aim
    while (platedetected() && adjust1 < 50) {
      pos++;
      ArmLift.write(pos);
      TheDelay =  50; runmainloop() ;
      adjust1++;
    }
    while (!platedetected() && adjust2 < 50) {
      pos--;
      ArmLift.write(pos);
      delay(100);
      TheDelay =  50; runmainloop() ;
    ...
    if (TryAgain && !minusdone) {
      Serial.println("STICKING THERE ---------------------------------------");
      //if this is a second attempt or more, go lower
      pos -= 3;
      ArmLift.write(pos);
      // and stick there
      adjust2 = 100; // no more adjustments
      adjust1 = 100; // no more adjustments
      minusdone = true; // do it only once
    }
    runmainloop();

    // if more than one plate sensor will detect it
    // then reengage within same loop and restart course correction
    if (toomanygrabbed && TryAgain) {
      // but if it's a second attempt, let the shit go out anyway
      Serial.println("");
      Serial.print("TOO MANY PLATES & 2nd Attempt... let the shit go");
      toomanygrabbed = true;
      break;
    }
  }

  PlateBarrier.write(PlateBarrierOpen);// must be open in any of previous scenarii

  if (plategrab()) {
    ArmLift.write(ArmLiftCurr + 4); // lift the arm to avoid bringing other plates due to adjustments
    ...
    // now move forward until the plate is either lost (timer)
    Limit = 45; // number of seconds above which plate not seen = lost
    ...
    // or until it is seen passing over the extractors

    while (!PlateIsOnTheWay() && !IsLastPlate() && elapsed < TimeLimit && !TooManyPlates()) {

      time = millis();
      elapsed = (unsigned long int)time - (unsigned long int)start ;
      countdown = (((unsigned long int)TimeLimit - (unsigned long int)elapsed)) / seconds;
      Serial.println("");
      Serial.print("Remaining time B4 declaring failure= "); Serial.print(countdown); Serial.print(" seconds");


      PlateBarrier.write(PlateBarrierOpen);
      TheDelay =  10; runmainloop() ;
    }
    // analyse the cause of interruption
    if (toomanygrabbed) {
      Serial.println("");
      Serial.print("TOO MANY PLATES! STARTING OVER");
      restartPlate();
    }
    // if time limit has passed it is failure, start over
    else if (elapsed > TimeLimit && !PlateIsOnTheWay()) {
      Serial.println("");
      Serial.print("FAILURE ___ PLATE NOT SEEN | PROBABLY LOST | STARTING OVER...");
      Serial.println("");
      restartPlate();
    }
...
void ReverseArm() {
  ...
  while (analogRead(ContactArmSensor) > 900 && elapsed < TimeLimit && powerIsOn && !stoptest) {
    time = millis();
    elapsed = (unsigned long int)time - (unsigned long int)start ;
    countdown = (((unsigned long int)TimeLimit - (unsigned long int)elapsed)) / seconds;
    TheDelay =  10; runmainloop() ;
  ...
  StopPlateExtractor();
  /// release pressure on the sensor
  while (analogRead(ContactArmSensor) < 400 && digitalRead(PWSW) == 0 && !stoptest) {
    analogWrite(pwmArm, 255);
    digitalWrite(ArmMotor1, LOW);
    digitalWrite(ArmMotor2, HIGH);
    TheDelay =  1; runmainloop() ;
...
void ReverseExtractor() {
  ...
  while (!ExtractorRetracted() &&  elapsed < TimeLimit && !stoptest) {
...
boolean platedetected() {
  ...
  //  Serial.println("");
  //  Serial.println("PLATE DETECTED ? "); Serial.print(yesno);
  //  Serial.println("");
...
boolean TooManyPlates() {
  ...
  if ( val <= detectval) {
    result = true;
    yesno = "YES";
    toomanygrabbed = true; // public boolean
  }
  else {
    result = false;
    yesno = "NO";
...
void SwitchOn() {
  if (!powerIsON()) {
    ...
    Serial.println("power is already on, doing nothing");
  }
}
