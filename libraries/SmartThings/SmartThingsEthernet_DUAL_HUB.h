...
#ifndef __SmartThingsEthernet_DUAL_HUB_H__ 
#define __SmartThingsEthernet_DUAL_HUB_H__
...
namespace st
{
	class SmartThingsEthernet_DUAL_HUB: public SmartThings
	{
	private:
	...
	public:
		...
		//*******************************************************************************
		SmartThingsEthernet_DUAL_HUB(IPAddress localIP, IPAddress localGateway, IPAddress localSubnetMask, IPAddress localDNSServer, uint16_t serverPort, IPAddress hubIP, uint16_t hubPort, IPAddress hubIP2, uint16_t hubPortb, SmartThingsCallout_t *callout, String shieldType = "EthernetShield", bool enableDebug = false, int transmitInterval = 100, bool DHCP = false);

		//Abrreviated STATIC version for Arduino + ESP-01 WiFiEsp library (no Gateway, Subnet Mask, or DNS can be user provided)
		SmartThingsEthernet_DUAL_HUB(IPAddress localIP, uint16_t serverPort, IPAddress hubIP, uint16_t hubPort, IPAddress hubIP2, uint16_t hubPortb, SmartThingsCallout_t *callout, String shieldType = "EthernetShield", bool enableDebug = false, int transmitInterval = 100, bool DHCP = false);
		...
		///   @param[in] enableDebug (optional) - Enable internal Library debug
		//*******************************************************************************
		SmartThingsEthernet_DUAL_HUB(uint16_t serverPort, IPAddress hubIP, uint16_t hubPort, IPAddress hubIP2, uint16_t hubPortb, SmartThingsCallout_t *callout, String shieldType = "EthernetShield", bool enableDebug = false, int transmitInterval = 100, bool DHCP = true);

		//*******************************************************************************
		/// Destructor 
		//*******************************************************************************
		~SmartThingsEthernet_DUAL_HUB();
