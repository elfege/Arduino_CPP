...
namespace st
{
  ...
  //*******************************************************************************
  // SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS Constructor - Static IP
  //*******************************************************************************
  SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS::SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS(String ssid, String ssid2, String password, IPAddress localIP, IPAddress localGateway, IPAddress localSubnetMask, IPAddress localDNSServer, uint16_t serverPort, IPAddress hubIP, uint16_t hubPort, IPAddress hubIP2, uint16_t hubPortb, IPAddress hubIP3, uint16_t hubPortc, SmartThingsCallout_t *callout, String shieldType, bool enableDebug, int transmitInterval) :
  SmartThingsEthernet_DUAL_SSID_3_HUBS(localIP, localGateway, localSubnetMask, localDNSServer, serverPort, hubIP, hubPort, hubIP2, hubPortb, hubIP3, hubPortc, callout, shieldType, enableDebug, transmitInterval, false),
  ...
  //*******************************************************************************
  // SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS Constructor - DHCP
  //*******************************************************************************
  SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS::SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS(String ssid, String ssid2, String password, uint16_t serverPort, IPAddress hubIP, uint16_t hubPort, IPAddress hubIP2, uint16_t hubPortb, IPAddress hubIP3, uint16_t hubPortc, SmartThingsCallout_t *callout, String shieldType, bool enableDebug, int transmitInterval) :
  SmartThingsEthernet_DUAL_SSID_3_HUBS(serverPort, hubIP, hubPort, hubIP2, hubPortb, hubIP3, hubPortc, callout, shieldType, enableDebug, transmitInterval, true),
  st_server(serverPort)
  {
    ssid.toCharArray(st_ssid, sizeof(st_ssid));
    ssid2.toCharArray(st_ssid2, sizeof(st_ssid2));
  ...
  // SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS Constructor - DHCP
  //*******************************************************************************
  SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS::SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS(uint16_t serverPort, IPAddress hubIP, uint16_t hubPort, IPAddress hubIP2, uint16_t hubPortb, IPAddress hubIP3, uint16_t hubPortc, SmartThingsCallout_t *callout, String shieldType, bool enableDebug, int transmitInterval) :
  SmartThingsEthernet_DUAL_SSID_3_HUBS(serverPort, hubIP, hubPort, hubIP2, hubPortb, hubIP3, hubPortc, callout, shieldType, enableDebug, transmitInterval, true),
  st_server(serverPort)
  {
  ...
  //*****************************************************************************
  //SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS::~SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS()
  //*****************************************************************************
  SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS::~SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS()
  {
  ...
  //*******************************************************************************
  /// Initialize SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS Library
  //*******************************************************************************
  void SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS::init(void)
  {
    ...
    Serial.print(F("hubIP3 = "));
    Serial.println(st_hubIP3);
    Serial.print(F("hubPortc = "));
    Serial.println(st_hubPortc);
    Serial.print(F("RSSI = "));
  ...
  //*****************************************************************************
  // Run SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS Library
  //*****************************************************************************
  void SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS::run(void)
  {
  ...
  //*******************************************************************************
  void SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS::send(String message)

  {
    ...
    if (st_client.connect(st_hubIP, st_hubPort))
    {
      ...
      waitForResponse(st_hubIP);

    }
    else {

      Serial.println(F("***********************************************************"));
      Serial.print(F("*****  CONNECTION FAILED TO HUB: "));Serial.print(st_hubIP); Serial.println(st_hubPort); 
      Serial.println(F("***********************************************************"));
    ...
    if (st_client.connect(st_hubIP2, st_hubPortb))
    {
      ...
      waitForResponse(st_hubIP2);

    }
    else {

      Serial.println(F("***********************************************************"));
      Serial.print(F("*****  CONNECTION FAILED TO HUB: "));Serial.print(st_hubIP2); Serial.println(st_hubPortb); 
      Serial.println(F("***********************************************************"));
    ...
    st_client.stop();
    delay(1);
    if (st_client.connect(st_hubIP3, st_hubPortc))
    {

      st_client.println(F("POST / HTTP/1.1"));
      st_client.print(F("HOST: "));
      st_client.print(st_hubIP3);
      st_client.print(F(":"));
      st_client.println(st_hubPortc);
      st_client.println(F("CONTENT-TYPE: text"));
      st_client.print(F("CONTENT-LENGTH: "));
      st_client.println(message.length());
      st_client.println();
      ...
      waitForResponse(st_hubIP3);

    }
    else {

      Serial.println(F("***********************************************************"));
      Serial.print(F("*****  CONNECTION FAILED TO HUB: "));Serial.print(st_hubIP3); Serial.println(st_hubPortc); 
      Serial.println(F("***********************************************************"));
  ...
  void SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS::waitForResponse(IPAddress ipclient)
  {
  ...
  void SmartThingsESP8266WiFi_DUAL_SSID_3_HUBS::Reconnect(void)
  {
    int count = 0;
