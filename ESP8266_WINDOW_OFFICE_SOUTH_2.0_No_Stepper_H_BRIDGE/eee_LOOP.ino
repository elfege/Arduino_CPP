/* NO LONGER IDENTICAL TO THE OTHER WINDOW USING SAME THREADED ROD BASED ACTUATOR */
void loop() {
  //loopTime = millis();
...
void safety() {
  ...
  if ((opening || closing) && needAdjustment)// if still running after x minutes, increase speed
  {
    if (state.loadFromRTC() && speedAdjustmentOccurences == 0) // if values are stored and it's the first attempt
    {
      ...
      unsigned cc = millis();
      while (isOpen() && millis() - Start < 120000 && !stopped) // wait 30000 secs max
      {
        if (millis() - cc >= 30000)
        {
          FINALSPEED = FINALSPEED + 10 <= LEARNING_SPEED_LIMIT + 200 ? FINALSPEED + 10 : FINALSPEED; // if still open means that FINALSPEED was too slow, so learn from it.
