...
void fwd(int v, int dist) {
  ...
  if ((SeeLineLeft() || SeeLineLeft()) && !flwln && !cliffOFF) {
    ...
    digitalWrite(LEFT1,  A);
    digitalWrite(LEFT2,  B);
    digitalWrite(RIGHT1, B);
    digitalWrite(RIGHT2, A);
...
void rvs(int v, int dist) {
  ...
  ledcWrite(pwmChannelL, rqSp);
  ledcWrite(pwmChannelR, rqSp);
  digitalWrite(LEFT1,  B);
  digitalWrite(LEFT2,  A);
  digitalWrite(RIGHT1, A);
  digitalWrite(RIGHT2, B);
