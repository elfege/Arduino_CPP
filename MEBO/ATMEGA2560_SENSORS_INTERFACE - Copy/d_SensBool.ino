...
int fusupVAL_L() {
  ...
  duration = pulseIn(echoPinUPL, HIGH);
...
int fusupVAL_R() {
  ...
  // Reads the echoPinUPL, returns the sound wave travel time in microseconds
  duration = pulseIn(echoPinUPR, HIGH);
...
boolean RearFrontOK() {
  boolean a = analogRead(RRIR) <= 300 &&  analogRead(RLIR) <= 300 &&  analogRead(FIR) <= 300;
  return a;
}
