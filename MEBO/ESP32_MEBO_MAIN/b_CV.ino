...
boolean check = true; // this one is called by any other function telling main loop to not run Stuck() test, mostly to prevent redundencies in roam()
...
unsigned long previousMillisLogs = millis();
unsigned long previousCurrentMeasure = millis();
unsigned long timeCountToPwOff;
unsigned long Start;
unsigned long lastST_Stop = millis();
unsigned long RstCtDwnStart = millis();
unsigned long RecTime = millis(); // stuck timer
unsigned long MvtStateMillis = millis(); // for turn delays
unsigned long loopTime = millis();
unsigned long StartSearch = millis(); // docking operations timer
