...
void initXMLhttp() {
  ...
  _server.on("/logs", []() {
    ...
    if (!logs)
    {
      UartSend("+logstrue-");  // logs must not run at the same time on both boards
    }
    else
    {
      UartSend("+logsfalse-");  // logs must not run at the same time on both boards
    }
  } );
  _server.on("/logsAtmega", []() {
    //simply turn off the logs on ATmega
    term.println("logsAtmega js cmd sending '+logsfalse-' to UART");
    UartSend("+logsfalse-");  // logs must not run at the same time on both boards
