  ...
  ************** *****************************************************/
  ...
  while (!STOPALL) {
    ...
    while (!STOPALL && !bumped && (frontIR_RVal() < IRlimit && frontIR_LVal() < IRlimit) && fusupVAL() > 30 && fusLVAL() > 5 && fusRVAL() > 5 && !GotStuck && millis() - StartingPoint < 10000)
    {
      ...
      else if ((BUMPL() && !BUMPR()) || fusupVAL_L() < 30)
      {
        rvs(170, 0);
        delay(400);
        right(200);
    ...
    if (!STOPALL) {
      ...
      if (side == "left") {
        ...
        StartHere = millis();
        simpleLeft(160); // no automated stop
        logged = false;
        while (!freeWay() && !STOPALL && !leftOBS() && millis() - StartHere < 5000 || millis() - StartHere < 100 && !GotStuck && !bumped)
        {
          if (!logged) {
          ...
          if (rearOBS())
          {
            fwd(170, 0);
            delay(400);
            simpleLeft(160);//resume initial cmd
          }
          if ((BUMPL() && !BUMPR()) || fusupVAL_L() < 30)
          {
            rvs(170, 0);
            delay(400);
            right(200);
        ...
        StartHere = millis();
        simpleRight(160); // no automated stop
        logged = false;
        while (!freeWay() && !STOPALL && !rightOBS() && millis() - StartHere < 5000 && !GotStuck && !bumped)
        {
          if (!logged) {
          ...
          if (rearOBS())
          {
            fwd(170, 0);
            delay(400);
            simpleRight(160);//resume initial cmd
          }
          if ((BUMPR() && !BUMPL()) || fusupVAL_R() < 30)
          {
            rvs(170, 0);
...
boolean freeWay() {
  return frontIR_RVal() < 100 || frontIR_LVal() < 100 && fusLVAL() > 500 && fusRVAL() > 500;
}
