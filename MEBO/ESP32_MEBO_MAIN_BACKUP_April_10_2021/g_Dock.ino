...
void execLineRvs()
{
  ...
  while (SeeLineLeftRear() && !STOPALL && !haspower() && !GotStuck /*&& !rearOBS()*/ && millis() - a < tlim) {
    ...
    right(170);
    b = millis();
    while (SeeLineLeftRear() && !STOPALL && !haspower() && !GotStuck /*&& !rearOBS()*/ && millis() - b < 500)
    {
      MainFunctions();
    }
    STOP();
  ...
  a = millis();
  while (SeeLineRightRear() && !STOPALL && !haspower() && !GotStuck /*&& !rearOBS()*/ && millis() - a < tlim) {

    Serial.println("Line Dock left");
    left(170);
    b = millis();
    while (SeeLineRightRear() && !STOPALL && !haspower() && !GotStuck /*&& !rearOBS()*/ && millis() - b < 500)
    {
      MainFunctions();
    }
    STOP();
