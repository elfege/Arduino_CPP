void dock() {
  ...
  int MaxV4Dist = 700; // max dist val for when robot gets closer relatively to rear obstacle
  ...
  while (!STOPALL && !haspower() && millis() - GStart < timer) {
    ...
    while (found && !haspower() && millis() - Start < 5000 && !STOPALL && !GotStuck) {
      ...
      if (!found && rightIRVal() < 700 && leftIRVal() < 700) { // if lost track, try to find it before breaking // only if not into corridor already
...
void avoidRear() {
  if (RRIRVal() >= 700 && !haspower() && !STOPALL) {
  ...
  if (RLIRVal() >= 700 && !haspower() && !STOPALL) {
...
void RetrievalMethods(int MdSp) {
  ...
  if (hastried < 30 && !haspower() && !STOPALL) {
    ...
    if ((gotoNext || GotStuck || hastriedunstuck >= 1) && ((frontIRVal() >= 700 || BUMP() || fusupVAL() <= 20) && (leftIRVal() >= 700 || rightIRVal()) >= 700) && !haspower() && !STOPALL) {
    ...
    if (hastried >= 4) {
      if ((frontIRVal() >= 700 || BUMP() || fusupVAL() <= 20) && (leftIRVal() >= 700 || rightIRVal() >= 700) && !haspower() && !STOPALL) {
...
void RandomSearch() {
  ...
  while (!Signal && !STOPALL && !haspower() && millis() - GS < 50000) {
    ...
    // if got stuck
      ...
      if (frontIRVal() > 700 && (leftIRVal() > 700 || leftIRVal() > 2000 ) && !Signal && !STOPALL) {
        unstuckRVS();
        GotStuck = false;
      }
